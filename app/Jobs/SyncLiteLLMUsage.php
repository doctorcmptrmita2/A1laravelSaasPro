...
class SyncLiteLLMUsage implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            // Get last sync time
            $lastSync = LiteLLMSyncLog::where('sync_type', 'usage')
                ->where('status', 'success')
                ->latest('completed_at')
                ->first();

            $startDate = $lastSync?->completed_at 
                ? $lastSync->completed_at->subMinutes(15)->format('Y-m-d') 
            ...
            foreach ($tenants as $tenant) {
                ...
                foreach ($apiKeys as $apiKeyId) {
                    // Fetch usage from LiteLLM
                    $usage = $litellmClient->getUsage($startDate, $endDate, $apiKeyId);
                    
                    if (!empty($usage)) {
                        // Usage data is already aggregated, we can store it in cache or update tenant stats
